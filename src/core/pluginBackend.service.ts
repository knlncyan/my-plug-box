...
class PluginBackendService {
  ...
  async assertCommandExposed(commandId: string, callerPluginId?: string): Promise<void> {
    const payload = callerPluginId
      ? { commandId, callerPluginId }
      : { commandId };
    await this.invokeApi<void>('assert_command_exposed', payload);
  }
