...
const viewModules = import.meta.glob('../plugins/**/views/*View.tsx');
...
function resolveViewModuleKey(componentPath: string): string {
...
function App() {
  ...
  useEffect(() => {
    ...
    async function loadComponent(): Promise<void> {
      const moduleKey = resolveViewModuleKey(nextParams.componentPath);
      const loader = viewModules[moduleKey] as
        | (() => Promise<{ default?: ComponentType<Record<string, unknown>> }>)
        | undefined;

      if (!loader) {
        setError(`Component not found: ${nextParams.componentPath}`);
        setComponent(null);
        return;
      }
