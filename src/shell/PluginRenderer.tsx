...
import { pluginRuntime } from '../core/pluginRuntime';
import type { ViewMeta } from '../core/pluginRuntime.protocol';
import type {
...
} from '../core/pluginViewRuntimeBridge.protocol';
  ...
  useEffect(() => {
    ...
    function postSnapshot(targetWindow: Window): void {
      const message: PluginViewRuntimeSnapshotMessage = {
        ...
        snapshot: pluginRuntime.getSnapshot(),
      };
        ...
        switch (message.action) {
          case 'getSnapshot': {
            postResponse(targetWindow, {
              ...
              success: true,
              result: pluginRuntime.getSnapshot(),
            });
            return;
          }
          case 'subscribe': {
            if (!unsubscribeSnapshot) {
              unsubscribeSnapshot = pluginRuntime.subscribe(() => {
          ...
          case 'executeCommand': {
            ...
            const result = await pluginRuntime.executeCommand(
              payload.commandId,
              payload.options,
              ...args
            );
          ...
          case 'setActiveView': {
            ...
            pluginRuntime.setActiveView((payload.viewId ?? null) as string | null);
            postResponse(targetWindow, {
              type: 'plugin-view-runtime-response',
              requestId: message.requestId,
              success: true,
          ...
          case 'activateForView': {
            ...
            await pluginRuntime.activateForView(payload.viewId);
            postResponse(targetWindow, {
              type: 'plugin-view-runtime-response',
              requestId: message.requestId,
              success: true,
